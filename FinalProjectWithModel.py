...
def build_finger_model():
    model = tf.keras.models.load_model("Amelia_model")
    ...
    model.fit(np.asarray(train, dtype=np.float32), np.asarray(train_labels), epochs=3, batch_size=8, verbose=1)#, callbacks=[checkpoint])
...
build_finger_model()
#load_and_run_model("Amelia_model")#"models/fingers_hub_1")
