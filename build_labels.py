...
def find_centre(img):
    corner_list = get_corners(sudoku_bounds(get_board(enhance_writing(img))))
    x = []
    y = []
    for element in corner_list:
        x.append(element[0])
        y.append(element[1])
    centre_x = sum(x)/len(x)
    centre_y = sum(y)/len(y)
    centre = [centre_x, centre_y]
    return centre

#os.listdir("fileshere") returns a list of strings representing files
