...
def read_labels_and_images():
    ...
    for filename in os.listdir(filepath):
        ...
        if random.random() < -0.95: continue # to train properly
    ...
    for filename in filenames:
        if 100*num_done > (checkpoint + check_interval) * len(label_map):
            checkpoint += check_interval
            print((int)(100 * num_done / len(label_map)), "% done")
        num_done += 1
        img = cv2.imread(filepath + filename)/255.0
...
#    model.compile(optimizer=tf.keras.optimizers.Adamax(), loss='mse', metrics=['accuracy']) #77%
    model.compile(optimizer=tf.keras.optimizers.RMSprop(), loss='mse', metrics=['accuracy']) #84%
    ...
    model.summary()
    #model.fit(np.asarray(train, dtype=np.float32), np.asarray(train_labels), epochs=3, batch_size=8, verbose=1) #to see what's happening in the training
    model.fit(np.asarray(train, dtype=np.float32), np.asarray(train_labels), epochs=5, batch_size=8, verbose=0) #to get rid of detail on training
    #model.save("sudoku_centre")
    print("Done training network.")

    # Predict
    p = model.predict(np.asarray(predict))
    num_correct, num_total = 0, 0
    results = [[0 for i in range(3)] for j in range(3)]
    for pr, pl in zip(p, predict_labels):
        if np.linalg.norm(pr - pl) <= 30: num_correct += 1
        num_total += 1
#        results[pl][pr] += 1
#        print("PR is: ", pr)
#        print("PL is: ", pl)
